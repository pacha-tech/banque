...
public class AccountService {
    ...
    public BigDecimal getBalance(String accountNumber) {
        Account account = accountInterface.findByAccountNumber(accountNumber)
                .orElseThrow(() -> new ResourceNotFoundException("Compte introuvable avec le numéro : " + accountNumber));
    ...
    public String withdraw(TransactionRequest request) {
        ...
        // Vérification du solde : balance.compareTo(amount) < 0 signifie balance < amount
    ...
    public Transaction transfer(TransfertRequest request) {
        Account sourceAccount = accountInterface.findByAccountNumber(request.getSourceAccountNumber())
                .orElseThrow(() -> new ResourceNotFoundException("Compte source introuvable : " + request.getSourceAccountNumber()));

        Account destAccount = accountInterface.findByAccountNumber(request.getDestAccountNumber())
                .orElseThrow(() -> new ResourceNotFoundException("Compte destination introuvable : " + request.getDestAccountNumber()));
    ...
    public List<TransactionHistory> getAccountHistory(String accountNumber) {
        ...
        return transactionInterface.findBySourceAccountIdAccountOrDestAccountIdAccountOrderByExecutedAtDesc(account.getIdAccount(), account.getIdAccount())
